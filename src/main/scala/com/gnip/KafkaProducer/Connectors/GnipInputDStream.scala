package com.gnip.KafkaProducer.Connectors
...
class StreamingGZIPInputStream(is: InputStream) extends GZIPInputStream(is) {
  private final val wrapped = is
  ...
  def onStart() {
    try {
      ...
      if (responseCode >= 200 && responseCode <= 299) {
        val reader = new BufferedReader(new InputStreamReader(new StreamingGZIPInputStream(inputStream), StandardCharsets.UTF_8))
        storeData(reader)
      } else {
